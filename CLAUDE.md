...
This repo is two scripts for turning a bare VPS into a Claude Code + herdr coding
...
2. Run `bootstrap.sh` once (via the curl one-liner in README.md, or a local checkout).
   ...
   installed.
3. After that, `start.sh` is the entry point for every session — not `claude` directly,
...
- `bootstrap.sh` installs: Claude Code CLI, herdr, `herdr integration install claude`,
...
- The `~/.bashrc` hook is the load-bearing piece: it makes every pane herdr spawns
  ...
  means "launch herdr" and "launch claude inside herdr" collapse into one script with no
  ...
  `claude`. Do not replace this with explicit `herdr agent start --kind claude --pane
...
- Don't add fleet/multi-agent-naming logic (NATO phonetic names, per-pane agent-kind
  selection, etc.) unless asked — out of scope for this repo's job, which is just "get
  Claude Code running via herdr on a fresh box."
